...
export default {
  interpret(pattern: string, metas?: { [k: string]: Function }, values?: { [k: string]: any }) {
    ...
    return pattern.replace(/<([A-Z]+)((?:\s+\w+="(?:""|[^"]+)*")*\s*)>/g, function(m, meta, strProps) {
      ...
      return meta === 'NOW' ? nowMoment.format(props.format) : metas?.hasOwnProperty(meta) ? metas[meta](values) : m;
    });
  },
